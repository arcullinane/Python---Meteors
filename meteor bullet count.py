...
def bullet_count(surf, text, size, x, y, img):
    img_rect = img.get_rect()
    img_rect.x = WIDTH -50
    img_rect.y = HEIGHT -20
    surf.blit(img, img_rect)
    font = pygame.font.Font(font_name, size)
...
def show_go_screen():
    ...
    draw_text(screen, "Arrow keys to move, Space key to fire", 22, WIDTH / 2, HEIGHT / 2)
    draw_text(screen, "Press any key to begin", 18, WIDTH / 2, HEIGHT * 0.75)
    draw_text(screen, "Andrew Cullinane", 12, WIDTH / 2, HEIGHT -20)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if keystate[pygame.K_RIGHT]:
            self.speedx = 8
        if keystate[pygame.K_SPACE] and player.bullets_left > 0:
            self.shoot()
...
bullet_mini_img = pygame.transform.scale(bullet_img, (5, 20))
...
while running:
    ...
    for hit in hits:
        ...
        if player.shield <= 0:
            ...
            player.lives -= 1
            player.shield = 100
            player.bullets_left = 100

    #check to see if player catches powerup
    hits = pygame.sprite.spritecollide(player, powerups, True)
    for hit in hits:
        if hit.type == "shield":
            player.shield += random.randrange(10, 30)
            player.bullets_left += 20
            if player.shield >= 100:
                player.shield = 100
        if hit.type == "gun":
            player.powerup()

    # player death and explosion finished
    ...
    bullet_count(screen, str(player.bullets_left), 18, (WIDTH - 20), (HEIGHT - 10), bullet_mini_img)
